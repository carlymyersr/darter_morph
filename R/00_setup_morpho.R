...
subset_coords_to_gdf <- function(coords_array, gdf_meta) {
  ...
  if (any(is.na(idx))) {
    missing <- gdf_meta$specimen[is.na(idx)]
    stop("Specimens missing from coordinate array: ", paste(missing, collapse = ", "))
  }
...
make_hulls <- function(df, group_col, x = "PC1", y = "PC2", min_n = 3) {
  split_df <- split(df, df[[group_col]], drop = TRUE)
  hulls <- lapply(split_df, function(d) {
    d <- d[stats::complete.cases(d[, c(x, y)]), , drop = FALSE]
    if (nrow(d) < min_n) return(NULL)
    d[chull(d[[x]], d[[y]]), , drop = FALSE]
  })
  hulls <- hulls[!vapply(hulls, is.null, logical(1))]
  if (length(hulls) == 0) return(df[0, , drop = FALSE])
  do.call(rbind, hulls)
}
...
MORPHO_OBJECTS <- c(
  ...
  "lm_names",
  "trim_names",
  "trim_dimnames_landmarks",
  "subset_coords_to_gdf",
  "make_hulls",
  "DROP_POINTS",
  "KEEP_POINTS",
  "coords_all",
