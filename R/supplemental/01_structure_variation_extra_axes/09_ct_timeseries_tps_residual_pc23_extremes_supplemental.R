# ============================================================
# scripts/CT3_TPS_residual_PC23_extremes.R
# RESIDUAL TPS (Connecticut River time series): PC2 and PC3 synthetic axis shapes + extreme individuals
#
#   - Uses coords_resid_CT3 from R/methods/01_specimen_sampling_study_design/03_subset_CT_timeseries.R
#   - Computes gm.prcomp(coords_resid_CT3)
...
#   - Saves PNG (+ optional PDF) to figures/CT3_TPS_RESID_PC23/
# ============================================================
...
# User settings
# ---------------------------
TPS_DIR   <- file.path("figures", "CT3_TPS_RESID_PC23")
...
if (!exists("coords_resid_CT3")) {
  stop("coords_resid_CT3 not found. Ensure R/methods/01_specimen_sampling_study_design/03_subset_CT_timeseries.R builds it.")
}
if (!exists("coords_CT3")) {
  stop("coords_CT3 not found. Ensure R/methods/01_specimen_sampling_study_design/03_subset_CT_timeseries.R builds it.")
}
if (!exists("gdf_CT3")) {
  stop("gdf_CT3 not found. Ensure R/methods/01_specimen_sampling_study_design/03_subset_CT_timeseries.R builds it.")
}
...
# RAW reference for CT time series
ref_CT3 <- mshape(coords_CT3)
ref_CT3 <- matrix(
  ref_CT3,
  ncol = 2,
...
ref_resid_CT3 <- mshape(coords_resid_CT3)
...
stopifnot(identical(rownames(ref_resid_CT3), rownames(ref_CT3)))
...
pc_axis_targets_resid <- list(
  ...
  PC3_minus = make_pc_axis_shape(coords_resid_CT3, pca_resid_CT3, pc_i = 3, sign = -1, k_sd = PC_K),
  PC3_plus  = make_pc_axis_shape(coords_resid_CT3, pca_resid_CT3, pc_i = 3, sign =  1, k_sd = PC_K)
)

for (nm in names(pc_axis_targets_resid)) {
  axis_num <- if (grepl("^PC2", nm)) 2 else 3
  pct      <- pct_resid_CT3[axis_num]
  ...
  save_tps_png(ref_CT3, target_on_raw, links_all, fn_png, title)
  
  if (isTRUE(SAVE_PDF)) {
    fn_pdf <- file.path(TPS_DIR, paste0("TPS_RESID_axisOnRaw_", nm, "_", PC_K, "SD.pdf"))
    save_tps_pdf(ref_CT3, target_on_raw, links_all, fn_pdf, title)
  }
}
...
pc_extreme_shapes_resid <- lapply(
  pc_extreme_idx,
  function(i) coords_resid_CT3[, , i, drop = TRUE]
)
...
for (nm in names(pc_extreme_shapes_resid)) {
  ...
  specimen_id <- dimnames(coords_resid_CT3)[[3]][pc_extreme_idx[[nm]]]
  specimen_year <- gdf_CT3$year[match(specimen_id, gdf_CT3$specimen)]
  specimen_group <- gdf_CT3$group[match(specimen_id, gdf_CT3$specimen)]
  
  title <- paste0(
    "SIZE-CORRECTED PCA: ", axis_lab, " ", ext_lab, " individual (",
    sprintf("%.1f", pct), "%)\n",
    specimen_id, " | ", specimen_group, " | year ", specimen_year, "\n",
    "Displayed on RAW mean (residual deformation applied)"
  )
