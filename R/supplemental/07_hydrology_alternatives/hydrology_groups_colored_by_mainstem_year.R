...
#
# Updated plotting:
#   - Hulls remain by variation_source_alt
#   - Mainstem individual points are colored by year
#   - Reservoir System and Tributaries retain single point colors
# ============================================================
...
# Preconditions
# ---------------------------
if (!exists("coords_gpa")) stop("coords_gpa not found. Did R/methods/02_landmark_acquisition_geometric_morphometrics/00_setup_morpho.R run fully?")
if (!exists("gdf")) stop("gdf not found. Did R/methods/01_specimen_sampling_study_design/01_build_metadata.R run fully?")
...
# Reservoir groups are retained as habitats.
gdf_vs <- gdf %>%
  filter(
    !is.na(habitat),
    (
...
# ---------------------------
# Point-color grouping:
# Mainstem points colored by year
# Reservoir + Tributaries keep single colors
# ---------------------------
pca_vs_df <- pca_vs_df %>%
  mutate(
    point_group = case_when(
      variation_source_alt == "Mainstem" & year == 1950 ~ "Mainstem_1950",
      variation_source_alt == "Mainstem" & year == 1956 ~ "Mainstem_1956",
      variation_source_alt == "Mainstem" & year == 1970 ~ "Mainstem_1970",
      variation_source_alt == "Reservoir System" ~ "Reservoir System",
      variation_source_alt == "Tributaries" ~ "Tributaries",
      TRUE ~ NA_character_
    ),
    point_group = factor(
      point_group,
      levels = c(
        "Mainstem_1950", "Mainstem_1956", "Mainstem_1970",
        "Reservoir System", "Tributaries"
      )
    )
  )

stopifnot(!any(is.na(pca_vs_df$point_group)))

point_palette <- c(
  "Mainstem_1950" = "#1B9E77",
  "Mainstem_1956" = "#D95F02",
  "Mainstem_1970" = "#7570B3",
  "Reservoir System" = "#E6AB02",
  "Tributaries" = "#666666"
)

point_labels <- c(
  "Mainstem_1950" = "Mainstem (CT 1950)",
  "Mainstem_1956" = "Mainstem (CT 1956)",
  "Mainstem_1970" = "Mainstem (CT 1970)",
...
# ============================================================
# PCA plotting helper
# Hulls by variation source; points by year/group
# ============================================================
make_vs_plot <- function(df, hull_df, xvar, yvar, xlab, ylab, subtitle_text) {
  ggplot(df, aes(x = .data[[xvar]], y = .data[[yvar]])) +
    ...
    scale_fill_manual(values = vs_palette, name = "Hull group") +
    scale_color_manual(
...
p_pc12 <- make_vs_plot(
  ...
  subtitle_text = paste0(
    "PC1 vs PC2 | Hulls = variation source | Mainstem points colored by year | ",
    "Allometry: shape ~ ", size_label
  )
)
...
p_pc23 <- make_vs_plot(
  ...
  ylab = pc3_lab,
  subtitle_text = paste0(
    "PC2 vs PC3 | Hulls = variation source | Mainstem points colored by year | ",
    "Allometry: shape ~ ", size_label
  )
)
...
p_pc34 <- make_vs_plot(
  ...
  ylab = pc4_lab,
  subtitle_text = paste0(
    "PC3 vs PC4 | Hulls = variation source | Mainstem points colored by year | ",
    "Allometry: shape ~ ", size_label
  )
)
...
pdf_path_pc12 <- file.path(FIG_DIR, "Fig_mainstem_alltime_vs_tribandres_PC12_mainstemByYear.pdf")
png_path_pc12 <- file.path(FIG_DIR, "Fig_mainstem_alltime_vs_tribandres_PC12_mainstemByYear.png")

pdf_path_pc23 <- file.path(FIG_DIR, "Fig_mainstem_alltime_vs_tribandres_PC23_mainstemByYear.pdf")
png_path_pc23 <- file.path(FIG_DIR, "Fig_mainstem_alltime_vs_tribandres_PC23_mainstemByYear.png")

pdf_path_pc34 <- file.path(FIG_DIR, "Fig_mainstem_alltime_vs_tribandres_PC34_mainstemByYear.pdf")
png_path_pc34 <- file.path(FIG_DIR, "Fig_mainstem_alltime_vs_tribandres_PC34_mainstemByYear.png")
