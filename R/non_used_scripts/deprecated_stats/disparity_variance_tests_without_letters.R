# ============================================================
# Scripts/disparity_variance_tests.R
#
# Run:
...
group_disparity_obs <- map_dfr(group_levels, function(g) {
...
p_disp <- ggplot(disp_boot_df, aes(x = group, y = disparity, fill = group)) +
  ...
  labs(
    ...
    subtitle = "Boxplots = bootstrap distributions; white points = observed Procrustes variance",
    x = NULL,
...
variance_df <- tibble(
  ...
  group = meta_use$group[match(names(bd$distances), meta_use$specimen)],
...
ct_disparity_obs <- map_dfr(ct_levels, function(g) {
...
p_ct_disp <- mean(perm_diff >= ct_disp_diff)

cat("\nCT disparity difference test p-value:", p_ct_disp, "\n")
...
ct_variance_df <- tibble(
  ...
  group = ct_meta$ct_group[match(names(bd_ct$distances), ct_meta$specimen)],
...
landscape_disparity_obs <- map_dfr(landscape_levels, function(g) {
...
p_landscape_disp <- ggplot(
  ...
  labs(
    ...
    subtitle = "CT_timeseries = CT 1950 + 1956 + 1970; white points = observed Procrustes variance",
    x = NULL,
    y = "Procrustes variance"
  ) +
  theme_classic(base_size = 12) +
