...
#
# Requires:
#   from R/methods/02_landmark_acquisition_geometric_morphometrics/00_setup_morpho.R:
...
#
#   from R/methods/01_specimen_sampling_study_design/01_build_metadata.R:
...
if (VERBOSE) cat("Running R/methods/01_specimen_sampling_study_design/02_subset_1950.R...\n")

# ---------------------------
# Preconditions
# ---------------------------
if (!exists("gdf"))         stop("gdf not found. Run source('R/methods/01_specimen_sampling_study_design/01_build_metadata.R') first.")
if (!exists("coords_gpa"))  stop("coords_gpa not found. Run source('R/methods/02_landmark_acquisition_geometric_morphometrics/00_setup_morpho.R') first.")
if (!exists("hab_palette")) stop("hab_palette not found. Run source('R/methods/01_specimen_sampling_study_design/01_build_metadata.R') first.")
if (!exists("curve_endpoints")) stop("curve_endpoints not found. Run source('R/methods/02_landmark_acquisition_geometric_morphometrics/00_setup_morpho.R') first.")
if (!exists("allometry_residuals")) stop("allometry_residuals() not found. Run source('R/methods/01_specimen_sampling_study_design/01_build_metadata.R') first.")
...
if (!exists("make_curve_index")) {
  stop("make_curve_index not found. Keep it in R/methods/02_landmark_acquisition_geometric_morphometrics/00_setup_morpho.R (Helper functions section).")
}
if (!exists("make_links_from_index")) {
  stop("make_links_from_index not found. Keep it in R/methods/02_landmark_acquisition_geometric_morphometrics/00_setup_morpho.R (Helper functions section).")
}
...
if (!("size_for_allometry" %in% names(gdf_1950))) {
  stop("gdf_1950 is missing size_for_allometry. Check R/methods/01_specimen_sampling_study_design/01_build_metadata.R.")
}
...
if (VERBOSE) {
  cat("\nObjects created by R/methods/01_specimen_sampling_study_design/02_subset_1950.R:\n")
